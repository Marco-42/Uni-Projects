...
massa = np.array([721.9, 1.988410e+30, 6.04568e+24, 6.4171e+23, 1.89858e+27, 5.6847e+26, 8.681e+25, 1.024e+26, 4.8685e+24, 3.302e+23], dtype=np.float64) #moon-earth system
...
# Funzione compatibile con Numba: niente np.delete, niente copy, niente del
@njit
def func(y, x, planet):
    ...
    # Calcola la forza degli altri pianeti senza np.delete/copy
    ...
    for i in range(y.shape[0]):
        ...
        if dist > 0:
            force += m2 * diff / (dist**3)
    f[3:] += constants.G * force
    return f

# Funzione compatibile con Numba: solo array, niente oggetti Python
@njit
def func_voy(y, voy):
    f = np.zeros(6, dtype=np.float64)
    r = voy[:3]
    f[:3] = voy[3:]
    # Sole
    acc = -massa[1]*constants.G*r/((np.linalg.norm(r))**3)
    # Pianeti
    for i in range(y.shape[0]):
        diff = y[i, :3] - r
        dist = np.linalg.norm(diff)
        if dist > 0:
            acc += massa[i+2]*constants.G*diff/(dist**3)
    f[3:] = acc
    return f
...
#   1977-08-21 15:32:00
    ...
    for i in range(tstep -1):
        ...
        dist_jupiter = np.linalg.norm(y[2, i, :3] - voy[i, :3])
        dist_saturn = np.linalg.norm(y[3, i, :3] - voy[i, :3])
        dist_earth = np.linalg.norm(y[0, i, :3] - voy[i, :3])

        min_dist = min(dist_jupiter, dist_saturn, dist_earth)

        if min_dist < ua / 5:
            dt = temp_orb(np.linalg.norm(y[3, 0, :3])) / (tstep * 20)
        else:
            dt = temp_orb(np.linalg.norm(y[3, 0, :3])) / (tstep * 5)      
    ...
    """
    for i in range(tstep-1):
        if(np.linalg.norm(y[2, i, :3]-voy[i, :3]) < ua/10):
            dt = temp_orb(np.linalg.norm(y[3, 0, :3]))/(tstep*6)
        else:
             dt = temp_orb(np.linalg.norm(y[3, 0, :3]))/(tstep*3)
        Y1 = np.array(voy[i])
        Y2 = Y1 +func_voy(planets[:, i, :], Y1)*dt/2
        Y3 = Y1 + func_voy(planets[:, i, :], Y2)*dt/2
        Y4 = Y1 + func_voy(planets[:, i, :], Y3)*dt
