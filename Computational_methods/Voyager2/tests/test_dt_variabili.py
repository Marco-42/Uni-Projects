...
#   1977-08-21 15:32:00
    ...
    for i in range(tstep -1):
        ...
        if(np.linalg.norm(y[2, i, :3]-voy[i, :3]) < ua/6 or np.linalg.norm(y[3, i, :3]-voy[i, :3]) < ua/6):
            dt = temp_orb(np.linalg.norm(y[3, 0, :3]))/(tstep*30)
        else:
            dt = temp_orb(np.linalg.norm(y[3, 0, :3]))/(tstep*4)
