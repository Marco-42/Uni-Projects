...
mass = np.array([825, 1.98841e+30, 3.301e+23, 4.8673e+24, 6.04568e+24, 6.4169e+23, 1.89858e+27, 5.6847e+26, 8.6811e+25, 1.02409e+26], dtype=np.float64)
...
def get_energy(y, spacecraft):
    ...
    # Computing the spacecraft cinetic energy
...
# ============== EXPLICIT EULERO =========================
# Function to execute the Explicit Eulero method to solve the differential equations for planets and spacecraft
@njit # To increase performance
def explicit_eulero(y, spacecraft, dt, i):
    """Explicit Eulero method: planets vector - spacecraft vector - delta_time - for index"""
    ...
    # Applying Explicit Eulero method to solve the spacecraft's differential equation
    Y1 = spacecraft[i]
    spacecraft_return = Y1 + func_spacecraft(y[:, i, :], Y1)*dt
...
# ============= VELOCITY VERLET ==========================
# Function to execute the Velocity Verlet method to solve the differential equations for planets and spacecraft
@njit # To increase performance
def velocity_verlet(y, spacecraft, dt, i):
    """Velocity Verlet method: planets vector - spacecraft vector - delta_time - for index"""

    y_return = np.zeros((9, 6), dtype = np.float64)

    # Applying velocity Verlet method to solve the planet's differential equation(for each planet)
    for k in range(9):
        Y1 = y[k, i,:]
        radius = Y1[:3] + Y1[3:]*dt + 0.5*func(y[:, i, :], k, Y1)[3:]*(dt**2)
        velocity = Y1[3:] + 0.5*(func(y[:, i, :], k, Y1)[3:] + func(y[:, i, :], k, np.hstack((radius, Y1[3:])))[3:])*dt
        y_return[k] = np.hstack((radius, velocity))
...
def plot_trajectory(planets, voy, tstep, animation_flag = True):
...
def main():
    ...
    last_print_time = datetime(1977, 10, 12)
    # Method selector
    method_type = {1: runge_kutta, 2: explicit_eulero, 3: "comparison"}
    ...
    print(f"Simulation start date: {TIME}")
    print(" ")
    ...
    # Voyager 2 trajectory simulation using Runge Kutta method
    if method == 1:
        ...
        for i in range(tstep - 1):
            ...
            # Print every 3 days
            #     if TIME - last_print_time >=timedelta(days=2):
            #         print(TIME)
            #         print(voy[i, 3:])
            ...
            # Update the planets and spacecraft position using velocity Verlet method
            planets[:, i+1], voy[i+1] = velocity_verlet(planets, voy, dt, i)

            if i == 0: 
                time.append(TIME)
                H.append(get_energy(planets[:, i, :], voy[i, :]))
            # Local update of time variable
            TIME = TIME + timedelta(seconds=dt)
            time.append(TIME)
            # Computing the total energy of the spacecraft
            H.append(get_energy(planets[:, i+1, :], voy[i+1, :]))
        ...
        fig = plt.figure(figsize=(10,6))
        plt.plot(time, H, color = "Blue", label="Voyager 2")
        plt.xlabel('Time [s]')
        plt.ylabel('Energy [J]')
        plt.title('Voyager 2 total energy')
        plt.legend()
        plt.grid(True, which='both', ls='--', alpha=0.5)
        plt.show() 
        
        plot_trajectory(planets, voy, tstep, animation_flag = True)
