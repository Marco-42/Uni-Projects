...
def func(y, x, planet): 
    ...
    y = np.delete(y, x, axis = 0)
    copied_lista = massa.copy()
    del copied_lista[x+2]
        
    single_planet_force = np.zeros((7, 3))
    single_planet_force =((y[:, :3]-r)/((np.linalg.norm((y[:, :3]-r), axis = 1))**3)[:, np.newaxis])*np.array(copied_lista[2:])[:, np.newaxis]
    f[3:] += constants.G*(np.sum(single_planet_force,  axis=0))
...
#   1977-08-21 15:32:00
    ...
    for i in range(tstep -1):
        ...
        if min_dist < ua / 5:
            dt = temp_orb(np.linalg.norm(y[3, 0, :3])) / (tstep * 70)
        else:
