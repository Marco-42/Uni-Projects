...
COMPLETE_ANALYSIS = 70 # Cluster size to full analysis for clusters
SEARCH_CLUSTER_TIME = 100
...
for a in angle:
    ...
    FILE_PATH = f"./data/angle_{a}/"
    SAVE_PATH = f"./plots/angle_{a}/"
...
# ===========================================================
    ...
    plt.savefig(SAVE_PATH + f"cluster_multiplicity_hist_uranium.png", dpi=300)
    plt.xlim(0, 25)
    ...
    df_clusters['ratio_x_y_bin'] = df_clusters['ratio_x_y'].round(1)

    # Plotting a colourplot (x axis = x/y ratio, y axis = multiplicity, color = number of clusters) for the cluster dimensions
    plt.figure(figsize=(9, 6))
    heatmap_data = df_clusters.pivot_table(
        index='multiplicity',
        columns='ratio_x_y_bin',
        values='pixels',
        aggfunc='size',
        fill_value=0,
    ).sort_index(axis=0).sort_index(axis=1)

    heatmap_values = np.ma.masked_where(heatmap_data.to_numpy() == 0, heatmap_data.to_numpy())
    positive_values = heatmap_values.compressed()
    if positive_values.size == 0:
        positive_values = np.array([1])

    x_edges = np.arange(len(heatmap_data.columns) + 1)
    y_edges = np.arange(len(heatmap_data.index) + 1)
    mesh = plt.pcolormesh(
        x_edges,
        y_edges,
        heatmap_values,
    ...
    tick_positions = np.arange(len(heatmap_data.columns)) + 0.5
    tick_values = np.array(heatmap_data.columns, dtype=float)
    tick_mask = np.isclose((tick_values * 5) % 1, 0)
    minor_tick_mask = ~tick_mask
    tick_labels = [f'{value:.1f}' for value in tick_values]

    plt.xticks(
        tick_positions[tick_mask],
        np.array(tick_labels)[tick_mask],
        rotation=45,
        ha='right'
    )
    plt.gca().set_xticks(tick_positions[minor_tick_mask], minor=True)
    plt.gca().tick_params(axis='x', which='minor', length=4, labelbottom=False)
    plt.yticks(np.arange(len(heatmap_data.index)) + 0.5, heatmap_data.index)
    plt.ylim(-1, len(heatmap_data.index) + 2)
    plt.colorbar(mesh, label='Counts')
    plt.xlabel('Ratio x/y')
    plt.ylabel('Multiplicity')
    plt.title(f'Cluster Dimensions - ANGLE {a}')
    plt.tight_layout()
    plt.savefig(SAVE_PATH + f"cluster_dimensions_heatmap_uranium.png", dpi=300)
    print(f"Graph {graph_increment} - Done")
    graph_increment += 1
    ...
    df_filtered = df_clusters[
        (df_clusters['ratio_x_y'] >= 0.7) &
        (df_clusters['ratio_x_y'] <= 1.3)
    ]
    ...
    plt.savefig(
        SAVE_PATH + f"cluster_density_hexbin_uranium.png",
        dpi=300,
        bbox_inches="tight"
    )
    ...
    if show_all_clusters:
        ...
        plt.grid(alpha=0.25)
        plt.savefig(SAVE_PATH + f"cluster_centers_map_uranium.png", dpi=300)
        print(f"Graph {graph_increment} - Done")
        graph_increment += 1
    ...
    plt.grid(alpha=0.25)
    plt.savefig(SAVE_PATH + f"full_cluster_map_uranium_first_{SEARCH_CLUSTER_TIME}s.png", dpi=300)
    print(f"Graph {graph_increment} - Done")
    graph_increment += 1
    ...
    plt.grid(alpha=0.25)
    plt.savefig(
        SAVE_PATH + f"Cluster_map_mult_{COMPLETE_ANALYSIS}_uranium_first_{SEARCH_CLUSTER_TIME}s.png",
        dpi=300
    )
    print(f"Graph {graph_increment} - Done")
    graph_increment += 1
    ...
    if show_all_multiplicity_clusters:
        ...
        plt.grid(alpha=0.25)
        plt.savefig(SAVE_PATH + f"full_cluster_map_mult_{COMPLETE_ANALYSIS}_uranium_all_time.png", dpi=300)
    ...
    plt.tight_layout()
    plt.yscale('log')
    plt.savefig(SAVE_PATH + f"total_cluster_multiplicity_uranium.png", dpi=300)
